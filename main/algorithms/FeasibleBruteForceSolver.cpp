...
FeasibleBruteForceSolver::FeasibleBruteForceSolver(std::string inputFilename, std::string outputFilename, int nrOfExecutions)
: KnapSackSolver(inputFilename, outputFilename, nrOfExecutions), currentCombination(knapSack.getNumOfItems()), bestCombination(), numOfCombinations(pow(2, knapSack.getNumOfItems())), failedCombinations()
{
	
}

void FeasibleBruteForceSolver::setUp(){
...
/**
	...
	* has been determined.
	*
	* However this algorithm remembers the previously tried bit patterns which exceeded the capacity. 
	* Thus it can skip those combinations which include all 1s of the previous ones.
	*/
void FeasibleBruteForceSolver::solve() {
	...
	for (unsigned long long int i = 0; i < numOfCombinations; ++i) {
		...
		//skip combinations which contain the 1s of a previously failed combination
		bool skip = false;
		for (int k = 0; k < failedCombinations.size() && !skip; k++){
			bool onesAreEqual = true;
			for (int m = 0; m < currentCombination.size() && onesAreEqual; m++){
				if (failedCombinations[k][m] == 1 && currentCombination[m] != 1)
					onesAreEqual = false;
			}
			if (onesAreEqual)
				skip = true;
		...
		if (tmpWeight <= maxWeight){
			if (tmpWorth > bestWorth){
				bestWorth = tmpWorth;
				bestCombination = currentCombination;
			}
		}
		//add current combination to failed combinations, since its weight exceeded capacity
		else{
			failedCombinations.insert(failedCombinations.begin(), currentCombination);
		}
	}
}

void FeasibleBruteForceSolver::tearDown(){
	KnapSackItem* items = knapSack.getItems();
	const int numOfItems = knapSack.getNumOfItems();
