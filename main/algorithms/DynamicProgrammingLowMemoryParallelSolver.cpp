...
DynamicProgrammingLowMemoryParallelSolver::~DynamicProgrammingLowMemoryParallelSolver(){
	delete[] integerItems;
}

// we can parallelize this, since this is not part of the measurement
void DynamicProgrammingLowMemoryParallelSolver::setUp(){
	// nothing to do here. integerItemsOfSolution is getting cleared in tearDown
}

void DynamicProgrammingLowMemoryParallelSolver::solve() {
...
void DynamicProgrammingLowMemoryParallelSolver::tearDown(){
...
void DynamicProgrammingLowMemoryParallelSolver::determineItemsOfSolutionRecursively(IntegerItem* items, int numOfItems, int capacity, int solution){
	if(capacity <= 0)
		return;
	...
	// Runs the two independent solveProblem calls in two seperate threads. By Default the sections block is waiting until all sections are finished.
